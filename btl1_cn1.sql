...
CREATE OR REPLACE FUNCTION tonghd (
    ...
    tong2   NUMBER := 0;
BEGIN
    SELECT COUNT(mahd) INTO tong1 FROM hoadon
    WHERE manv = v_manv;
...
BEGIN
    ...
    IF ( dem >0 ) THEN
        UPDATE g_qlkho
        SET soluong = soluong + v_soluong, ngaynhapkho = current_date
        WHERE magiay = v_magiay;
    ELSE
        INSERT INTO g_qlkho VALUES ( 'CN01', v_magiay, v_soluong, current_date );
        INSERT INTO g_qlbh VALUES ( 'CN01', v_magiay, 'Con Hang', 0 );
    END IF;
    COMMIT;
END;
...
-- Tìm xem Mã HD n?m ? CN1 hay CN2
    SELECT COUNT(mahd) INTO dem_mahd FROM hoadon
    WHERE mahd = v_mahd AND macn = 'CN01' AND makh = v_makh;
    IF ( dem_mahd > 0 ) THEN
-- Mã HD ? CN1
...
-- Ki?m tra hoá ??n ?ã mua giày này ch?a, n?u r?i thì c?p nh?t s? l??ng, ch?a thì insert vào CTHD 
            SELECT COUNT(magiay) INTO tim FROM cthd
            ...
            IF ( tim > 0 ) THEN
                ...
                WHERE mahd = v_mahd AND magiay = v_magiay;
            ELSE
                INSERT INTO cthd VALUES ( v_mahd, v_magiay, v_soluong );
            END IF;
        ELSE
-- Thông báo không ?? hàng
    ...
    ELSE
--Mã HD ? CN2
        SELECT COUNT(mahd) INTO dem_mahd FROM cn2_btl1.hoadon@dbl_cn2
        WHERE mahd = v_mahd AND macn = 'CN01' AND makh = v_makh;
        IF ( dem_mahd > 0) THEN
-- Mã HD ? CN2
-- Ki?m tra s? l??ng hàng trong kho CN1 ?? xu?t kho bán hàng 
            SELECT COUNT(magiay) INTO dem
            FROM g_qlkho
            WHERE magiay = v_magiay;
            SELECT soluong INTO soluong FROM g_qlkho
            WHERE magiay = v_magiay;
...
-- Ki?m tra hoá ??n ?ã mua túi xách này ch?a, n?u r?i thì c?p nh?t s? l??ng, ch?a thì insert vào CTHD 
                SELECT COUNT(magiay) INTO tim FROM cn2_btl1.cthd@dbl_cn2
                WHERE mahd = v_mahd AND magiay = v_magiay;
                IF ( tim > 0 ) THEN
                    ...
                    WHERE mahd = v_mahd AND magiay = v_magiay;
                ELSE
                    INSERT INTO cn2_btl1.cthd@dbl_cn2 VALUES ( v_mahd, v_magiay, v_soluong);
                END IF;
            ELSE
-- Thông báo không ?? hàng
                raise_application_error(-20000, 'Khong du hang de xuat kho');
...
FROM
    giay                      g
    JOIN cn2_btl1.g_qlbh@dbl_cn2   qlbh ON g.magiay = qlbh.magiay
WHERE
    qlbh.tinhtrang = 'Con hang'
MINUS
SELECT
    g.*
FROM
    giay     g
    JOIN g_qlbh   qlbh ON g.magiay = qlbh.magiay
WHERE
    qlbh.tinhtrang = 'Con hang';
...
FROM
    (
        ...
        UNION
        SELECT * FROM cn2_btl1.nhanvien@dbl_cn2
    ) nv
    JOIN (SELECT * FROM hoadon
        UNION
        SELECT * FROM cn2_btl1.hoadon@dbl_cn2
...
SELECT mausac, SUM(soluong)
FROM giay g,
    (
        SELECT * FROM cthd, hoadon hd
        WHERE cthd.mahd = hd.mahd
        UNION
        SELECT * FROM cn2_btl1.cthd@dbl_cn2 cthd, cn2_btl1.hoadon@dbl_cn2 hd
        WHERE
            cthd.mahd = hd.mahd
    ) hd
WHERE
    ...
    AND hd.makh IN (
        SELECT makh FROM
            (
                ...
                UNION
                SELECT makh, tuoi
                FROM cn2_btl1.khachhang@dbl_cn2
            )
        WHERE
...
FROM
    (
        SELECT hd.mahd, SUM(hd.soluong) tsl
        FROM
            (
                SELECT cthd.mahd, ngayhd, magiay, soluong
                FROM cn2_btl1.hoadon@dbl_cn2 hd, cn2_btl1.cthd@dbl_cn2 cthd
...
SELECT magiay, SUM(tsl)
FROM
    (
        SELECT magiay, SUM(soluong) tsl
        FROM cn2_btl1.cthd@dbl_cn2 ct1, cn2_btl1.hoadon@dbl_cn2 hd1
        WHERE ct1.mahd = hd1.mahd AND TO_CHAR(hd1.ngayhd, 'YYYY') = 2021
        GROUP BY magiay
        UNION
        SELECT magiay, SUM(soluong) tsl
        FROM cthd ct2, hoadon   hd2
        WHERE ct2.mahd = hd2.mahd AND TO_CHAR(hd2.ngayhd, 'YYYY') = 2021
        GROUP BY magiay
    ) ct
GROUP BY MAGIAY
HAVING 
...
SELECT
    a.magiay, b.sl sl_nu, c.sl sl_nam
FROM
    (
        SELECT DISTINCT hd.magiay
        FROM
            (
                SELECT * FROM khachhang
                UNION
                SELECT * FROM cn2_btl1.khachhang@dbl_cn2
            ) kh
            JOIN (
                SELECT * FROM hoadon JOIN cthd ON hoadon.mahd = cthd.mahd
                UNION
                SELECT *
                FROM cn2_btl1.hoadon@dbl_cn2 JOIN cn2_btl1.cthd@dbl_cn2 ON hoadon.mahd = cthd.mahd
            ) hd ON kh.makh = hd.makh
        WHERE kh.gioitinh = 'Nu'
        INTERSECT
        SELECT DISTINCT hd.magiay
        FROM
            (
            ...
            JOIN (
                SELECT * FROM hoadon JOIN cthd ON hoadon.mahd = cthd.mahd
                UNION
                SELECT * FROM cn2_btl1.hoadon@dbl_cn2 JOIN cn2_btl1.cthd@dbl_cn2 ON hoadon.mahd = cthd.mahd
            ) hd ON kh.makh = hd.makh
        WHERE kh.gioitinh = 'Nam'
    ) a,
    (
        SELECT hd.magiay, SUM(hd.soluong) sl
        FROM
            (
                SELECT * FROM khachhang
                UNION
...
WHERE a.magiay = b.magiay AND a.magiay = c.magiay;

--cau 10
SELECT DISTINCT ( makh )
FROM cn1_btl1.hoadon@dbl_cn1 hd1, cn1_btl1.cthd@dbl_cn1 ct1, giay g1
WHERE hd1.mahd = ct1.mahd AND ct1.magiay = g1.magiay 
    AND thuonghieu NOT IN (
        SELECT DISTINCT ( thuonghieu )
        FROM giay g
        WHERE giatien > 500000
            AND NOT EXISTS (
                SELECT * FROM cn1_btl1.cthd@dbl_cn1   ct2, cn1_btl1.hoadon@dbl_cn1 hd2
                WHERE g.magiay = ct2.magiay AND hd2.mahd = ct2.mahd AND hd2.mahd = hd1.mahd
            )
    );
...
--Toi uu cau truy van
--Cho bi?t nh?ng nhân viên(Mã nhân viên, H? tên, SODT) và tên chi nhánh c?a kho có l??ng trên 5 tri?u ?ã nh?ng ?ôi 
--giày có giá t? 1 tri?u cho các khách hàng Nam
...
---
explain plan for 
select /* GATHER_PLAN_STATISTICS */ nv.manv, nv.tennv, nv.sodt, cn.tencn
FROM
    nhanvien    nv,
    hoadon      hd,
    cthd,
...
WHERE
    ...
    AND g.magiay = cthd.magiay
    AND kh.makh = hd.makh
    AND cn.macn = hd.macn
    AND nv.luong > 5000000 
    AND g.giatien >= 1000000
    AND kh.gioitinh = 'Nam';
---
explain plan for 
--câu truy v?n sau khi t?i ?u hóa
select nv.manv, nv.tennv, nv.sodt, tcn.tencn
from(
    select cn.tencn, nhd.manv
    from(
        select
        mhd.macn, mhd.manv
        from(
            select hd.mahd, hd.manv, hd.makh, hd.macn
            from(
                select mahd, manv, makh, macn
                from hoadon
            ) hd join (
                select makh
                from khachhang
            ...
            from(
                ...
                select magiay
                from giay
                where giatien >= 1000000
            ) g ON cthd.magiay = g.magiay
        ) ct ON mhd.mahd = ct.mahd
    )nhd join ( 
        select macn, tencn
        from chinhanh
    )cn on nhd.macn = cn.macn
)tcn join (
    select manv, tennv, sodt
    from nhanvien
    where luong > 5000000
    )nv on tcn.manv = nv.manv;
